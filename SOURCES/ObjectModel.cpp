...
ObjectModel::ObjectModel(const Model* model) {
	p_model = std::shared_ptr<const Model>(model);

	Vector3f v = Vector3f(&p_model->vertices()[0]);
	p_bounding_box = std::pair<Vector3f, Vector3f>(v,v);
	for (unsigned int i = 3; i < p_model->vertices().size(); i += 3) {
		v = Vector3f(&p_model->vertices()[i]);
